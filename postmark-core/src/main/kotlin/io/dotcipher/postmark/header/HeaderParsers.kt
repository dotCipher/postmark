...
private val DATE_TIME_OFFSET_REGEX = Regex("(%s %s %s)".format(DATE_PATTERN, TIME_PATTERN, OFFSET_PATTERN))

private val DATE_TIME_OFFSET_FORMATTER = DateTimeFormatter.RFC_1123_DATE_TIME
...
internal fun String.parseAsOffsetDateTime(): OffsetDateTime {
    val match = DATE_TIME_OFFSET_REGEX.matchEntire(this)
            ?: throw IllegalArgumentException("Unable to parse OffsetDateTime from: $this")
    return match.destructured
            .let { (dateTimeString) -> OffsetDateTime.parse(dateTimeString, DATE_TIME_OFFSET_FORMATTER) }
}
